...
chosenFlies = [89,92,85,138,139,140];
...
for fly = 1:length(chosenFlies)
    ...
    if saveOutput == 1
        ...
        allStruct = struct;
        allStruct.allData = allData;
        allStruct.allDatetimes = allDatetimes;
        allStruct.thisCSVs = thisCSVs;
        if doAcInac
            allStruct.inacBinaryProc = inacBinaryProc;
        end
        save( [thisVidFolder,filesep,'fly',num2str(flyOnDay),'_mov'], 'allStruct'); %Note: This switch may cause issues for other scripts/functions that rely on root architecture for allData etc
    
        %Save extra data that relates to all blocks of fly
        if exist('overSave')
            save( [thisVidFolder,filesep,'fly',num2str(flyOnDay),'_overSave'], 'overSave');            
        end
    else
        disp(['(Not saved)'])
    end
