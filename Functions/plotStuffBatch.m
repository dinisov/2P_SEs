function plotStuffBatch(R, iter, outputDirectory)

brainImage = R.brainImage;

%% collapsed across volumes (time)

subDirectory = fullfile(outputDirectory);
...
if isfield(R,'r')
    % plot r
    plotRBatch(R.r, iter, subDirectory);
    
    % plot r overlayed in fly's brain (thresholded by significance)
    signLevel = 0.05;
    plotBrainBatch(R.r, brainImage, iter, signLevel, subDirectory);
end
