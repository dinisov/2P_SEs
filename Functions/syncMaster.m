...
%Mk 11 - Rolling design upgrades
%Mk 12 - Support for PCI experiments
...
arguments
    ...
    options.rollingAnalysis double = -1 %Deprecated, probably
    ...
    options.outputDirectory string = "C:\Users\uqmvan13\ANALYSIS\2p\2P_RESULTS_5"
    ...
    options.pciAnalysis double = 0; %Whether to process for PCI data (Lacks PTB, modified bleachOut, etc)
end
functionAlity = 1;
%}
%{
%This block exists in the event that syncMaster is to be run not as a function, by disabling the header and footer elements
    %It is mostly deprecated as a coding tool in favour of using breakpoints
...
for fly = 1:length(FLIES) %Need to check this actually does multiple flies
    ...
    for thisBlock = [BLOCKS]
        ...
        syncStruct.CI.frameData = temp{1}; %Frame count
        syncStruct.DI.bleachOutData = temp{2}; %Arduino state (Battery, SEs, etc) OR red LED on/off (PCI)
        syncStruct.DI.frameOutData = temp{3}; %Frame state?
        if ~isempty(temp{4}) && ~isempty(temp{5})
            syncStruct.AI.FrameSpike = temp{4}; %PTB DAQ associated
            syncStruct.AI.Iterator = temp{5}; %Ditto
        ...
        if ~isempty(temp{6}) && ~pciAnalysis
            syncStruct.AI.Photodiode = temp{6}; %Photodiode, LED panel associated
            ...
            if ~pciAnalysis
                ...
                disp(['(Not retrieving photodiode data because PCI)'])
            end
            hasPhotData = 0;
        end
        clear temp
        ...
        if useShortcut == 0 || isShortcutting == 0        
            ...
            if ~pciAnalysis
                ...
                %bleachEnd does not indicate same thing in PCI experiment, so less than useful
            else
                pciFrameEndInd = find( diff(syncStruct.CI.frameData) == 1, 1, 'last' ); %Within 1 sample of ending
                    %Note that this indicates imaging end, NOT arduino end, which is unindicated in current Arduino PCI script
                pciFrameStartInd = find( diff(syncStruct.CI.frameData) == 1, 1, 'first' ); %First instance of red LED stim (for PCI)
            end
        end

        %Testatory plot for ThorSync structures
        %{
            %Note: Under normal (i.e. non-PCI) conditions, bleachOut ('redLEDPin' on Arduino) indicates arduino active state
            %frameData always indicates the frame number, increasing then plateauing at end of imaging
            %frameOutData seems to indicate frame imaging state or similar?
        figure
        hold on
        plot( syncStruct.DI.bleachOutData )
        plot( double(syncStruct.CI.frameData)/20000 ) %Arbitrary
        plot( syncStruct.DI.frameOutData )
        legend({'bleachOut','frameData','frameOutData'})
        %}
        ...
        if hasPTB %Probably critical at this point
            ...
            if isfield( matParamStruct.matSave, 'blockDesign' ) && matParamStruct.matSave.blockDesign == 1
                ...
                %Quick report wrt rolling analysis
                if thisBlock.doRolling
            ...
            %Check if iterator used
            useIterator = 1; %Will be overridden momentarily according to params, if necessary
            if isfield( matParamStruct.matSave, 'useIterator' )
                useIterator = matParamStruct.matSave.useIterator;
            else
                useIterator = 1; %Default yes
            end
        ...
        %DAQ
        if hasDaqData && ~isShortcutting && ~pciAnalysis %Added pciAnalysis here; Might cause issues
            %freak
            ...
            %taste
            if forceNoIterator 
                disp(['(Iterator data will be forcibly ignored)'])
            end
            if ~useIterator 
                disp(['(Iterator data was not used in PTB)'])
            end
                ...
                for eros = 1:size( errorSpikes,2 )
                    ...
                    if ifiRectificationMode == 0
                        ...
                        elseif ismember( thisErrorSpike, rectifiedErrorSpikes )
                            ...
                            continue
                        elseif (~forceNoIterator && useIterator) && ... %Add iterator ignore shortcut boolean
                                abs( (frameLOCS( thisErrorSpike+1 ) - frameLOCS( thisErrorSpike )) - (itLOCS( thisErrorSpike+2 ) - itLOCS( thisErrorSpike+1 )) ) < 0.05*nanmean(temp2)
                        %"Framespike delayed (or broad), but phase with iterator maintained"
                            ...
                            continue
                        elseif (~forceNoIterator && useIterator) && ...
                                ( frameLOCS( thisErrorSpike+1 ) - frameLOCS( thisErrorSpike ) ) < 0.75*nanmean(temp2) && ...
                        ...
                        elseif frameLOCS(thisErrorSpike) - frameLOCS(thisErrorSpike-1) < 0.25*nanmean(temp2) || frameLOCS(thisErrorSpike+1) - frameLOCS(thisErrorSpike) < 0.25*nanmean(temp2)
                            ...
                            crash = yes
                        elseif (~forceNoIterator && useIterator) && ...
                            frameLOCS(thisErrorSpike) - frameLOCS(thisErrorSpike-1) < 1.1*nanmean(temp2) && frameLOCS(thisErrorSpike+1) - frameLOCS(thisErrorSpike) < 1.5*nanmean(temp2) && ...
            ...
            if (batteryDesign == 0 && (nSpikes == targetINum || nSpikes == targetINum-1) ) || ( batteryDesign == 1 )
                ...
                if ~batteryDesign
                    ...
                    if (~forceNoIterator && useIterator)
                        ...
                        if exist('preKnowledge')
                            disp(['Using earlier knowledge for ending case (frameSpike n -> ',num2str(preKnowledge),')'])
                            lastUseful = length(frameLOCS) + preKnowledge;
                        else
                            disp(['(No iterator; Assuming terminal pattern Foxtrot 2 Beta 2 [frameSpike n - 1])'])
                            lastUseful = length(frameLOCS) - 1;
                        end
                    end
                    ...
                    if (~forceNoIterator && useIterator)
                        ...
                        disp(['(No iterator; Assuming terminal pattern Foxtrot 2 Beta 3 [frameSpike n])'])
                        lastUseful = length(frameLOCS);
                    end
        ...
        if ~isShortcutting %Only calculate if actually loaded h5 data
            ...
            if ~pciAnalysis
                ...
                tsPCILastFrameTime = inferTimes( pciFrameEndInd );
            end
            ...
            if ~pciAnalysis
                ...
                tsPCILastFrameTime = shortStruct.tsPCILastFrameTime;
                pciFrameEndInd = shortStruct.pciFrameEndInd;
                pciFrameStartInd = shortStruct.pciFrameStartInd;
            end
        ...
        if ~pciAnalysis
            ...
            disp([ 'Estimated duration of PCI imaging: ', num2str((tsPCILastFrameTime)/60),'m (',num2str(tsPCILastFrameTime),'s)'])
        end
        ...
        %% PCI equivalent of Phot
        if ~isShortcutting && pciAnalysis
        %Incorporate red LED data
                %NOTE: INEFFICIENT; PROBABLY REPLACE WITH INTERPOLATED/SMALLER VERSION SOON
            %if hasPhotData
            redLEDData = syncStruct.DI.bleachOutData;
            %end
        elseif isShortcutting && pciAnalysis %&& batteryDesign 
            redLEDData = shortStruct.redLEDData;
        end
        ...
        if saveShortcut == 1 && isShortcutting ~= 1
            ...
            if ~pciAnalysis
                ...
                shortStruct.tsPCILastFrameTime = tsPCILastFrameTime;
                shortStruct.pciFrameEndInd = pciFrameEndInd; %Reminder: Last imaging frame, not specifically end of red LED application
                shortStruct.pciFrameStartInd = pciFrameStartInd;
            end
            ...
            if pciAnalysis
                shortStruct.redLEDData = redLEDData;
            end
        ...
        if hasData && ( hasPTB || pciAnalysis )

            if ~pciAnalysis %Normal
                ...
                imStimStart = find( frameOnsetIndices - pciFrameStartInd > 0 , 1 , 'first' ); %Might theoretically be better to find min, rather than first after?
                imStimEnd = find( frameOnsetIndices - pciFrameEndInd > 0 , 1 , 'first' );
            end
            %QA
            if ( isempty( imStimStart ) || isempty( imStimEnd ) ) && ~pciAnalysis
            ...
            %}
              ...
              preBaselineExists = 0;
              if ~pciAnalysis && ( find( imStimTerp ~= 5, 1, 'first' ) ~= 1 && imStimTerp(1) == 5 )
              ...
              if ~pciAnalysis && ( includesBlanks && blankHandleMode == 1 )
              ...
              if ( bendyBlockDesign == 0 && batteryDesign == 0 ) && ~pciAnalysis
                  ...
                  if ~unsiphonedSEs %'Normal', siphoned SEs
                      ...
                      %(The following block generates in effect discontinuous time, as a matter of optimal volume collection)
                      ...
                      %}
                      %New, element-based reindicisation
                      %{
                      nStimuli = options.nBack;

                      nVolPerEl = floor( size(dataStimTrim,3)/ length(randomSeqActual) );
                      disp(['Calculated vol/s per element: ',num2str(nVolPerEl)])
                      %QA
                      if nVolPerEl < 1 %Not really an acceptable state of affairs; Can occur naturally if stimuli frequency high and imaging rate slow
                          ['## Error: Insufficient volumes to be collected per rolling element ##']
                          crash = yes
                      end

                      samPoints = [ 1 : length(randomSeqActual) ]; %Note: Unlike the above, old implementation, here samPoints correspond to each element of randomSeq
                        %Also note linspace from 1, not from 0
                      lastVolFrameInds = stimFrameInds( samPoints ); %Redundant, because yields stimFrameInds in its entirety?
                      newFrameInds = nan( nVolPerEl, size(lastVolFrameInds,2) );
                      for vol = 1:nVolPerEl
                          newFrameInds( vol, : ) = ceil( lastVolFrameInds ) - nVolPerEl + vol; %nVol rows, n of blocks cols
                      end
                      ...
                      dataStimTrim = dataStimTrim;
                      imStimTerp = imStimTerp;
                      volTimes = volTimes; %Symbolic
                  end
                  %}
              ...
              if ~pciAnalysis
                  ...
                  %"Not battery design AND blanks present or block design AND not siphoning SEs"
                  ...
                  elseif ~batteryDesign && unsiphonedSEs  %As indicated
                      disp(['-# Not siphoning SEs data by request #-'])
                        %NOTE: NOT TESTED FOR ROLLING
                  elseif batteryDesign %Ditto
                      disp(['-- Battery design; Siphoning unnecessary --'])
                  elseif bendyBlockDesign == 0 && ~unsiphonedSEs && ~any( imStimTerp == 5 ) %Rolling, with siphoning, no blanks present
                      disp(['-- Rolling design, siphoning requested but no blanks detected --'])
                  else
                      ['## unknown case ##']
                      crash = yes
                  end
              else
                  disp(['-- PCI analysis; No siphoning --'])    
                  phaseShifted = 0;
                  stillRollable = 1;
                  hasSiphoned = 0;
              end
              ...
              if dataSource == -1
                  ...
                  if ~pciAnalysis && ( ~batteryDesign && ~unsiphonedSEs ) %Rolling, Block, etc
                      %abort
                  ...
                  if exist('bendyBlockDesign') && hasPTB
                    %BLOCKS( thisFlyRowInd ).bendyBlockDesign =  matParamStruct.matSave.blockDesign;
                    BLOCKS( thisFlyRowInd ).bendyBlockDesign =  matParamStruct.matSave.blockDesign;
                  end
              ...
              if ~pciAnalysis && ~batteryDesign && isempty( BLOCKS( thisFlyRowInd ).randomSequence ) && ( allowRandomSequenceEmpty == 0 && ~unsiphonedSEs )
        ...
        %% Save a video
            %Note: Completely untested for PCI analysis
        if doVid && hasData
