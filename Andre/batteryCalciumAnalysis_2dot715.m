...
% V2...Mk5 - (28/10/25), titles of plots more informative (include regionName), crossFly plots no longer generated if only single fly, ROIOffset now dynamic, no longer crashes if stats not appropiate (fly numbers)
...
%         15 - Matt fixing of things that crash
...
allGraphs = 1; %option to turn on plots unrelated to thesis
...
for currentState = behavStates
    ...
    if behavSep
        if currentState == -1
            ...
            disp(['Analysis commencing for ', char(stateNames(currentState)), ' periods only'])
        end
    end
    ...
    for region = 1:size(roiList,2)
                ...
                if currentBlock.nStimuli == -2 || currentBlock.nStimuli == -5
                    ...
                    if roughTiming %will likely be removed, added so I can quickly access opto data exclusively doesn't factor in any analysis variables except for ROI, will likely go under if analyseAverage bool when ready to see other conditions
                        ...
                        for i = LRInds(1) + 1:2:LRInds(end)+1
                            ...
                            LRVols(LRCount, [size(stimVol, 2) + 1 : size(stimVol, 2) + size(nonStimVol, 2)], 4) = nonStimBehav;
        ...
        if conditionChoice(2)
            ...
            for batFly = 1:size(blockNums, 2)  %find(blockNums == 2)
                ...
                allFlyLR(flyCount, :, 1, blockNums(batFly) ) = nanmean([squeeze(LRVols(:,:,1,1)); squeeze(LRVols(:,:,1,2))]);
    ...
    if size(roiList, 2) == 1
        ...
        for block = 1:size(regionisedAmps, 3)
            ...
            for stimType = [2,3]
                ...
                if size(chosenFlies, 2) < 4 %size(LRData, 2) < 4
...
if automatedSaveFigures
    automatedPlotSaver( autoFigPath , [], 'clearOldFigures', 1 )
end