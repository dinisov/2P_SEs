...
% V2...Mk5 - (28/10/25), titles of plots more informative (include regionName), crossFly plots no longer generated if only single fly, ROIOffset now dynamic, no longer crashes if stats not appropriate (fly numbers)
% V2...Mk6 - (29/10/25) option to use BT over pixelSubtraction added, behavioural files now loaded for behavData, method for behavioural state separation implemented.
% V2...Mk7 - (), titles of plots more informative (include behavioural state).
%         15 - Matt fixing of things that crash/bad (Wrong block grabbing, NaN blocks, )
...
%roiList = {['leftMB'], ['bg']};
%roiList = {'leftMB'};
roiList = {['leftBulb'],['leftStrand'],['ring'],['ringRight'], ['bg']};
...
automatedSaveFigures = 1; %Whether to use a function to save all figures
if automatedSaveFigures
    autoFigPath = 'C:\Users\uqmvan13\2p\ANDRE2P_RESULTS';
    additionalFigParams = ['EPG_6s'];
end



debugOverSave = 0; %Whether to save overStruct to a specific folder
if debugOverSave
    overSaveFolder = 'C:\Users\uqmvan13\2p\ANDRE_DEBUG';
    savSeed = [num2str(nansum(chosenFlies)),'-',num2str(nansum(cell2mat(chosenBlocks)))];
        %Note: Seed generated bootleg from sum of chosenFlies-chosenBlocks
        %Theoretically can be shared by multiple combinations of flies/blocks, but v. unlikely
end
...
if size(chosenFlies, 2) < 4
    disp('** WARNING: not enough flies for statistical analyses **')
end

%generating ROIoffset based on # ROIs chosen
figure
tempFig = bar([1:size(roiList, 2);1:size(roiList, 2)]);
RoiOffset = [tempFig.XOffset];
close

% overStruct = struct; %structure to allow cross-ROI analysese

saveVars = who; %saving all variables to this point
saveVars = [saveVars;'saveVars';'currentState']; %adding necessary variables to stored variables

for currentState = behavStates
    clearvars('-except', saveVars{:}); %clearing variables generated in loop

    overStruct = struct; %structure to allow cross-ROI analyses
    loopVars = who; %saving all variables for roi loop
    loopVars = [loopVars; 'loopVars'; 'region']; %adding same varaibles as was case before state loop

    if behavSep
        if currentState == -1
            disp('Analysis commencing, no behavioural state separation')
        else
            disp([char(10),char(10),'Analysis commencing for ', char(stateNames(currentState)), ' periods only'])
        end
    end


    for region = 1:size(roiList,2)
        clearvars('-except', loopVars{:} ); %resetting variables to what they were before loop
        ...
        % end
        % else
        disp([char(10),'Region: ',regionName])
        if ROI
            roiStruct = struct;
            roiCount = 1;
        ...
        for fly = 1:length(chosenFlies)
            ...
            % for b = 1
            for b = 1:nBlocks %Note: Data structure reference frame, not actual block number
            %for b = chosenBlocks{fly}
                %currentBlock = thisFlyBlocks(b,:); %See above incorrect
                thisBlock = chosenBlocks{fly}(b);
                blockInd = find(thisFlyBlocks.Block == thisBlock);
                if isempty(blockInd)
                    ['## Alert: No block #',num2str(b),' found for fly ',num2str(thisFly),' ##']
                    crash = yes
                end
                currentBlock = thisFlyBlocks(blockInd,:);
                %witch
                disp(['-- Processing fly #',num2str(thisFly),' block #',num2str(thisBlock),...
                    ' (Dataset ',num2str(fly),'s ',num2str(b),'th stored block) --'])
                ...
                if currentBlock.nStimuli == -2 || currentBlock.nStimuli == -5
                    ...
                    flyFolder = ['fly', num2str(currentBlock.FlyOnDay), '_exp', num2str(currentBlock.Block), '_', currentDate];
                    ...
                    behavDirectory = [dataDirectory, filesep, currentDate, filesep, flyFolder];

                    %Loading files
                    BTData = csvread(btDataDirectory); %BT
                    load(btAncillaryDirectory); % btAncillaryArray
                    load(matDataDirectory); % matSave
                    %behav files
                        %Reminder: behavData is in i reference frame
                    if currentState == -1 %no separation
                        behavData = repmat(-1, 1, matSave.sequenceLength); % -1 denotes no behav separation, so making fictive behavioural data where all data gets included
                    else
                        if ~useBT
                            if exist([behavDirectory, filesep, 'behavSequence.mat']) ~= 2
                                % disp('## behavioural data (pixelSubtraction) not found ##')
                                disp(['## behavioural data (pixelSubtraction) not found for fly ', num2str(FLIES(fly).BLOCKS(b).flyNum), ' block ', num2str(b), ' ##'])
                                crash = yes;
                            else
                                load([behavDirectory, filesep, 'behavSequence.mat']); % behavData (will be manual for testing, then will load, eventually pull from FLIES.BLOCKS)
                                behavData = savStruct.acInac.thisInacBinaryInterp;
                            end
                        else 
                            if exist([behavDirectory, filesep, 'btBehav.mat']) ~= 2
                                % disp('## behavioural data (BT-based) not found ##')
                                disp(['## behavioural data (BT-based) not found for fly ', num2str(FLIES(fly).BLOCKS(b).flyNum), ' block ', num2str(b), ' ##'])
                                crash = yes;
                            else
                                behavFile = load([behavDirectory, filesep, 'btBehav.mat']);
                                behavData = behavFile.nirg; %change for BT
                            end
                        end
                        %Plot
                        %(Moved below to incorporate inds information)
                        %{
                        figure
                        plot(behavData)
                        ylim([-0.5,1.5])
                        title(['Fly ',num2str(thisFly),' B',num2str(currentBlock.Block),' (',strrep(flyID,'_',' '),') - States: ',num2str(unique(behavData)')])
                        set(gcf,'Name',['Fly ',num2str(thisFly),' B',num2str(currentBlock.Block),' behavData'])
                        %}
                    end
                    ...
                    if size(BTData, 2) < 11
                        ...
                        if isfield(matSave, 'batSequence') %feel like this isn't correct - find will give an array of the indices, not an integer
                            ...
                            LRInds = [matSave.optoElements + 1 : elementsLR]; %Note: In i reference frame, as might be expected
                    ...
                    %this will eventually be done earlier in load region
                    % TEMP - testing sequence of behaviour (will hopefully split LR by half - can comp with left vs right)
                    % if currentState == -1
                    %     behavData = repmat(1, 1, matSave.sequenceLength); % -1 denotes no behav separation, so making fictive behavioural data where all data gets included
                    % else
                    %     %load
                    %     behavData = ones(1, matSave.sequenceLength);
                    %     % behavData(LRInds(1): LRInds(1) + ceil((LRInds(end) - LRInds(1))/2) ) = 0; % dynamic testing? ;)
                    %     behavData(LRInds(1): LRInds(1) + ceil((LRInds(end) - LRInds(1))/2) + 3 ) = 0; % dynamic testing? ;)
                    % end
                    ...
                    if doSmooth
                        ...
                        smoothVols(find(isnan(nanmean(rawVols, [1,2])))) = nan;
                    end
                    ...
                    %dontcry
                    %Behav/stim plot if applicable
                    figure
                    for graphTyp = 1:2
                        subplot(2,1,graphTyp)

                        if graphTyp == 1
                            timeRef = [1:length(behavData)];
                        else
                            timeRef = iTimes;
                        end

                        plot(timeRef,behavData)
                        hold on

                        %Opto
                        line([timeRef(optoInds(1)),timeRef(optoInds(1))],[-0.125,1.125],'Color','c')
                        line([timeRef(optoInds(end)),timeRef(optoInds(end))],[-0.25,1.25],'Color','c')
                        line([timeRef(optoInds(1)),timeRef(optoInds(end))],[-0.1,-0.1],'Color','c','LineStyle',':','LineWidth',2)
                        %LR
                        line([timeRef(LRInds(1)),timeRef(LRInds(1))],[-0.125,1.125],'Color','g')
                        line([timeRef(LRInds(end)),timeRef(LRInds(end))],[-0.25,1.25],'Color','g')
                        line([timeRef(LRInds(1)),timeRef(LRInds(end))],[-0.1,-0.1],'Color','g','LineStyle',':','LineWidth',2)
                        %Lum
                        line([timeRef(lumInds(1)),timeRef(lumInds(1))],[-0.125,1.125],'Color','b')
                        line([timeRef(lumInds(end)),timeRef(lumInds(end))],[-0.25,1.25],'Color','b')
                        line([timeRef(lumInds(1)),timeRef(lumInds(end))],[-0.1,-0.1],'Color','b','LineStyle',':','LineWidth',2)
                        %Freq
                        line([timeRef(freqInds(1)),timeRef(freqInds(1))],[-0.125,1.125],'Color','r')
                        line([timeRef(freqInds(end)),timeRef(freqInds(end))],[-0.25,1.25],'Color','r')
                        line([timeRef(freqInds(1)),timeRef(freqInds(end))],[-0.1,-0.1],'Color','r','LineStyle',':','LineWidth',2)
                        if currentState ~= -1
                            ylim([-0.5,1.5])
                        else
                            ylim([-1.5,1.5])
                        ...
                        if graphTyp == 1
                            ...
                            xticks([0:60:nanmax(timeRef)])
                            xlabel('Time (s)')
                        end
                        titleStr = ['Fly ',num2str(thisFly),' B',num2str(currentBlock.Block),' (',strrep(flyID,'_',' '),') - ',...
                            ' C-Opto, G-LR, B-Lum, R-Freq'];
                        unBehav = unique(behavData);
                        if numel(unBehav) >= 2
                            for stat = 1:size(unBehav,1)
                                titleStr = [titleStr,' ',num2str(unBehav(stat)),'->',char(stateNames(unBehav(stat)))];
                            end
                        end
                        title(titleStr)
                    end
                    ...
                    if roughTiming %will likely be removed, added so I can quickly access opto data exclusively doesn't factor in any analysis variables except for ROI, will likely go under if analyseAverage bool when ready to see other conditions
                        ...
                        for vol = 1:size(volTimes, 2)
                            if vol <= abs(shiftAmount) && shiftAmount < 0
                                ...
                                thisIVols(:, :, vol - lastVol, 1) = rawVols(:, :, vol + shiftAmount); %actual vols (calcium data)
                        ...
                        %% LR traces
                        repCount = nan(5, 7); %matrix to keep track of how many reps remain for each condition - only relevant for behavioural splicing where reps can be lost [opto(1)/LR(2)/Lum(3)/freq(4)/blank(5), stimType (e.g. combined/left/right)], opto currently included but not used
                        %^^ may have to move earlier if/when opto is included
                        disp('LR vols being collected')
                        % LRVols = nan(size(unique(LRInds), 2)/2, size(allIVols, 3), 3); % [rep, vol, calc/phot/time]  %maybe 3rd dim for time and photo?
                        LRVols = nan(size(unique(LRInds), 2)/2, size(allIVols, 3), 4); % [rep, volume, calcium/phot/time/behav]
                        LRImages = nan(64, 64, size(allIVols, 3), size(unique(LRInds), 2)/2);
                        LRCount = 1;

                        for i = LRInds(1) + 1:2:LRInds(end)+1
                            ...
                            if i+1 == LRInds(end) + 1 %7603 %LRInds(end)
                            ...
                            %behavioural
                            stimBehav = repmat(behavData(i), 1, size(stimVol, 2)); %using stimVol as an anchor for # vols per i
                            nonStimBehav = repmat(behavData(i+1), 1, size(nonStimVol, 2));
                            ...
                            LRVols(LRCount, [size(stimVol, 2) + 1 : size(stimVol, 2) + size(nonStimVol, 2)], 1) = nonStimVol; %Data
                            %photodiode
                            LRVols(LRCount, [1: size(stimPhot, 2)], 2) = stimPhot;
                            LRVols(LRCount, [size(stimPhot, 2) + 1 : size(stimPhot, 2) + size(nonStimPhot, 2)], 2) = nonStimPhot; %Phot
                            %time
                            LRVols(LRCount, 1: size(stimTime, 2), 3) = stimTime;
                            LRVols(LRCount, size(stimTime, 2) + 1 : size(stimTime, 2) + size(nonStimTime, 2), 3) = nonStimTime; %Time
                            %behav
                            LRVols(LRCount, 1:size(stimVol, 2), 4) = stimBehav;
                            LRVols(LRCount, [size(stimVol, 2) + 1 : size(stimVol, 2) + size(nonStimVol, 2)], 4) = nonStimBehav; %Behavioural state(?)
                        ...
                        for rep = 1:size(LRVols, 1)
                            ...
                            if isempty(thisLRSize)
                                dropCountLR = dropCountLR + 1;
                            else
                        ...
                        %separating LRVols into
                        unorganisedLRVols = LRVols;
                        LRVols = nan(size(unorganisedLRVols, 1)/2, size(unorganisedLRVols, 2), size(unorganisedLRVols, 3), 2);
                        LRVols(:,:,:,1) = unorganisedLRVols(1:end/2, :,:); %lefts
                        LRVols(:,:,:,2) = unorganisedLRVols(end/2 + 1:end, :, :); %rights

                        unorganisedTimes = segmentedLRTimes;
                        segmentedLRTimes = nan(size(unorganisedTimes, 1)/2, size(unorganisedTimes, 2), 2);
                        segmentedLRTimes(:,:,1) = unorganisedTimes(1:end/2, :); %lefts
                        segmentedLRTimes(:,:,2) = unorganisedTimes(end/2+1:end, :); %rights

                        % splicing by state
                        statedLRVols = nanmean(LRVols(:,:,4,:), 2);
                        for side = 1:2
                            LRVols(squeeze(statedLRVols(:,:,:,side)) ~= currentState, :, :, side) = nan;
                            segmentedLRTimes(squeeze(statedLRVols(:,:,:,side)) ~= currentState, :, side) = nan;
                            %should probably also kill LRImages here too
                        end

                        %counting number of reps per condition
                        repCount(2,2) = sum(~isnan(nanmean(LRVols(:,:,1,1), 2))); %left
                        repCount(2,3) = sum(~isnan(nanmean(LRVols(:,:,1,2), 2))); %right
                        repCount(2,1) = repCount(2,2) + repCount(2,3); %combined
                        ...
                        lumVols = nan(size(unique(lumInds), 2)/(2*7), size(allIVols, 3), 4, 7); %7 hardcoded but pretty safe bet given how stimulus script coded
                        lumCount = ones(1, 7);
                        tic
                        ...
                        for i = lumInds(1) + 1:2:lumInds(end) + 1 %7605:2:7884 %lumInds hardcoded [83]
                            ...
                            %behav
                            lumVols(lumCount(lumIndex), 1: size(stimVol, 2), 4, lumIndex) = stimBehav;
                            lumVols(lumCount(lumIndex), size(stimVol, 2) + 1 : size(stimVol, 2) + size(nonStimVol, 2), 4, lumIndex) = nonStimBehav;
                        ...
                        % splicing by state
                        statedLumVols = nanmean(lumVols(:,:,4,:), 2);
                        for lum = 1:size(lumVols, 4)
                            lumVols(squeeze(statedLumVols(:,:,:,lum)) ~= currentState, :, :, lum) = nan;
                            segmentedLumTimes(squeeze(statedLumVols(:,:,:,lum)) ~= currentState, :, lum) = nan;
                        end

                        %counting number of reps per condition
                        for lum = 1:size(lumVols, 4)
                            repCount(3, lum) = sum(~isnan(nanmean(lumVols(:,:,1,lum), 2)));
                        end
                        ...
                        freqVols = nan(matSave.numFreq*2, size(allIVols, 3), 4, size(matSave.batFrequencies, 2));
                        ...
                        for i = sepFreqInds(1,:)%7885:8925 %hardcoded freqInds - may want this to be first inds (presentFereqInds
                            if ismember(freqSequence(i), matSave.batFrequencies) %ismember(freqSequence(i-1), matSave.batFrequencies)
                                ...
                                if freqSequence(i) == 1
                                    ...
                                    currentBehav = repmat(behavData(freqI), 1, size(currentVol, 1));

                                    stimVol = [stimVol, currentVol'];
                                    stimPhot = [stimPhot, currentPhot'];
                                    stimTime = [stimTime, currentTime'];
                                    stimBehav = [stimBehav, currentBehav];
                                ...
                                for imagingI = [i, i+1]
                                    if freqSequence(i-1) ~= 1 || freqSequence(i-1) == 1 && imagingI ~= i %empirical adjustment for 1Hz
                                        ...
                                        currentNonStimBehav = repmat(behavData(imagingI), 1, size(currentNonStimVol, 1));
                                        ...
                                        nonStimBehav = [nonStimBehav; currentNonStimBehav']; %not sure why nonStim is vertical and stim was horizontal, but keeping it for the moment because seems to work
                                    end
                            ...
                            if fullI
                                ...
                                %behav
                                freqVols(freqCount(freqIndice), 1:size(stimVol, 2), 4, freqIndice) = stimBehav;
                                freqVols(freqCount(freqIndice), size(stimVol, 2) + 1: size(stimVol, 2) + size(nonStimVol, 1), 4, freqIndice) = nonStimBehav';
                        ...
                        %making time consistent for each frequency
                        ...
                        % splicing by state
                        statedFreqVols = nanmean(freqVols(:,:,4,:), 2);
                        for freq = 1:size(freqVols, 4)
                            freqVols(squeeze(statedFreqVols(:,:,:,freq)) ~= currentState, :, :, freq) = nan;
                            segmentedFreqTimes(squeeze(statedFreqVols(:,:,:,freq)) ~= currentState, :, freq) = nan;
                        end

                        %counting number of reps per condition
                        for freq = 1:size(freqVols, 4)
                            repCount(4, freq) = sum(~isnan(nanmean(freqVols(:,:,1,freq), 2)));
                        end
                        ...
                        %disp(['Analysis complete for fly ', num2str(chosenFlies(fly)), ' block', num2str(b)])
                        disp(['-- Analysis complete for fly ', num2str(chosenFlies(fly)), ' block', num2str(chosenBlocks{fly}(b)),' --'])

                        %% blanks
                        blankVols = nan(size(blankInds, 2), size(allIVols, 3), 4);
                        blankCount = 1;
                        for i = blankInds
                            if i == blankInds(1) || i == blankInds(end) % opto blank and end of recording- empirically only 1 i in length
                                ...
                                blankVols(blankCount, :, 4) = repmat(behavData(i), 1, size( squeeze(nanmean(allIVols(:,:,:,i), [1,2])), 1) );
                            else
                                ...
                                %behavioural
                                firstBlankBehav = repmat(behavData(i), 1, size(firstBlankVol, 1));
                                secondBlankBehav = repmat(behavData(i+1), 1, size(secondBlankVol, 1));
                                ...
                                %behav
                                blankVols(blankCount, 1:size(firstBlankVol, 1), 4) = firstBlankBehav;
                                blankVols(blankCount, size(firstBlankVol, 1) + 1: size(firstBlankVol, 1) + size(secondBlankVol, 1), 4) = secondBlankBehav;
                            end
                        ...
                        % splicing by state
                        statedBlankVols = nanmean(blankVols(:,:, 4), 2);
                        blankVols(squeeze(statedBlankVols) ~= currentState, :, :) = nan;
                        segmentedBlankTimes(squeeze(statedBlankVols) ~= currentState, :) = nan;

                        %counting number of reps per condition
                        repCount(5, 1) = sum(~isnan(nanmean(blankVols(:,:,4), 2)));

                        batStruct.blocks(structCount).blankVols = blankVols;
                        batStruct.blocks(structCount).segmentedBlankTimes = segmentedBlankTimes;

                        batStruct.blocks(structCount).repCount = repCount;
                        ...
                        if figChoice(3) == 1
                            for side = [1,2]
                                ...
                                if side == 1
                                    ...
                                    title(['f:', num2str(thisFly), ' b:', num2str(thisBlock), ' Luminosity Transients Left'])
                                elseif side == 2
                                    %                     title(['Luminosity Transients Right'])
                                    %title(['f:', num2str(thisFly), ' b:', num2str(b), ' Luminosity Transients Right'])
                                    title(['f:', num2str(thisFly), ' b:', num2str(thisBlock), ' Luminosity Transients Right'])
                                end
                            end
                        end
                        ...
                        if figChoice(4) == 1
                            ...
                            for side = [1,2]
                                ...
                                if side == 1
                                    ...
                                    title(['f:', num2str(thisFly), ' b:', num2str(thisBlock), ' Frequency Transients Left'])
                                elseif side == 2
                                    %title(['f:', num2str(thisFly), ' b:', num2str(b), ' Frequency Transients Right'])
                                    title(['f:', num2str(thisFly), ' b:', num2str(thisBlock), ' Frequency Transients Right'])
                                end
                            end
                        end
                    ...
                    ['## Alert: Non-battery (probably) block detected ##'] %...based on nStimuli
                    crash = yes
                end
            ...
            clear thisBlock %Prevent bleed
        end
        clear thisFly %Prevent bleed
        ...
        if conditionChoice(2)
            ...
            % allFlyLR = nan( size(chosenFlies, 2), size(LRVols, 2), 5, max(unique(blockNums)) ); %[fly, volume, calc/phot/time/activeOnly/inactive, block]
            ...
            for batFly = 1:size(blockNums, 2)  %find(blockNums == 2)
                ...
                allFlyLR(flyCount, :, 1, blockNums(batFly) ) = nanmean([squeeze(LRVols(:,:,1,1)); squeeze(LRVols(:,:,1,2))]); %Note: Fails if different numbers of timepoints present
                % allFlyLR(flyCount, :, 2, blockNums(batFly)) = nanmean(LRVols(:,:, 2));
                allFlyLR(flyCount, :, 2, blockNums(batFly)) = nanmean([squeeze(LRVols(:,:,2,1)); squeeze(LRVols(:,:,2,2))]);
                % allFlyLR(flyCount, :, 3, blockNums(batFly)) = nanmean(segmentedLRTimes(:, :));
                allFlyLR(flyCount, :, 3, blockNums(batFly)) = nanmean([ squeeze(segmentedLRTimes(:,:,1)) ; squeeze(segmentedLRTimes(:,:,2)) ] );

                sideNum = size(LRVols, 1)/2;
                %left
                allFlyLeft(flyCount, :, 1, blockNums(batFly) ) = nanmean(LRVols(:,:,1, 1));
                allFlyLeft(flyCount, :, 2, blockNums(batFly)) = nanmean(LRVols(:,:, 2, 1));
                allFlyLeft(flyCount, :, 3, blockNums(batFly)) = nanmean(segmentedLRTimes(:, :, 1));

                %right
                allFlyRight(flyCount, :, 1, blockNums(batFly) ) = nanmean(LRVols(:,:,1, 2));
                allFlyRight(flyCount, :, 2, blockNums(batFly)) = nanmean(LRVols(:,:, 2, 2));
                allFlyRight(flyCount, :, 3, blockNums(batFly)) = nanmean(segmentedLRTimes(:, :, 2));

                %behavSeparation
                % repAcInac = nanmean(LRVols(:,:,4), 2);
                % allFlyLR(flyCount, :, 4, blockNums(batFly) ) = nanmean(LRVols(find(repAcInac==0),:,1));
                % for state = 1:max(unique(behavData))
                %     stateReps = find(nanmean(LRV))
                % end

                % for rep = 1:size(LRVols, 1)
                %     if nanmean(LRVols(rep,:,4)) ~= 0 %active
                %         allFlyLR(flyCount, :, 4, blockNums(batFly) ) = nanmean()
                %     else %inactive (any time fly wasn't inactive for entire repitition)
                %         allFlyLR
                %     end
                % end
                ...
                if allGraphs
                    ...
                    %}
                    sideSEM = nanstd(LRVols, [], 1)/sqrt(size(LRVols, 1));
                    ...
                    figure
                    hold on
                    plot(segmentedLRTimes(:, :, 1)', LRVols(:,:, 1,1)')
                    plot(nanmean(segmentedLRTimes(:, :, 1)), nanmean(LRVols(:, :, 1, 1)), 'LineWidth', 3, 'Color', 'k');
                    errorbar(nanmean(segmentedLRTimes(:, :, 1)), nanmean(LRVols(:, :, 1, 1)), sideSEM(:,:,1,1) , '.', 'Color','k')
                    ...
                    ylabel('mean fluorescence')
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    set(gcf, 'Name', ['fly ', num2str(batStruct.blocks(batFly).fly), ' block ', num2str(blockNums(batFly)), ' lefts - ', regionName, ' Behav: ', char(stateNames(currentState))], 'NumberTitle', 'off', 'Renderer', 'painters')
                    title(['fly ', num2str(batStruct.blocks(batFly).fly), ' block ', num2str(blockNums(batFly)), ' lefts - ', regionName, ' Behav: ', char(stateNames(currentState))])


                    %right
                    figure
                    hold on
                    plot(segmentedLRTimes(:, :, 2)', LRVols(:, :, 1, 2)')
                    plot(nanmean(segmentedLRTimes(:, :, 2)), nanmean(LRVols(:, :, 1, 2)), 'LineWidth', 3, 'Color', 'k');
                    errorbar(nanmean(segmentedLRTimes(:, :, 2)), nanmean(LRVols(:, :, 1, 2)), sideSEM(:,:,1,2), '.', 'Color', 'k')
                    yLims = get(gca, 'ylim');
                    fill([0 0 1.089 1.089], [yLims(1), yLims(2), yLims(2), yLims(1)], 'b', 'FaceAlpha', 0.3)
                    % legend([p1,p2], {'left', 'right'}) <- this is correct legend
                    ...
                    ylabel('mean fluorescence')
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    set(gcf, 'Name', ['fly ', num2str(batStruct.blocks(batFly).fly), ' block ', num2str(blockNums(batFly)), ' Rights - ', regionName, ' Behav: ', char(stateNames(currentState))], 'NumberTitle', 'off', 'Renderer', 'painters')
                    title(['fly ', num2str(batStruct.blocks(batFly).fly), ' block ', num2str(blockNums(batFly)), ' Rights - ', regionName, ' Behav: ', char(stateNames(currentState))])
                end
            end
            toc
            ...
            if size(chosenFlies, 2) > 1
                ...
                for batBlock = 1:size(allFlyLR, 4) %find(blockNums == blockType)
                    ...
                    set(gcf, 'Name', ['cross-fly LR traces - ', regionName, ' block ', num2str(batBlock), ' Behav: ', char(stateNames(currentState))], 'NumberTitle', 'off', 'Renderer', 'painters')

                    yLims = get(gca, 'ylim');
                    fill([0 0 1.089 1.089], [yLims(1), yLims(2), yLims(2), yLims(1)], 'b', 'FaceAlpha', 0.3)
                    title(['cross-fly LR traces - ', regionName, ' block ', num2str(batBlock), ' Behav: ', char(stateNames(currentState))])
                    %if currentState == 1
                    %    darkness
                    %end
                    ...
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    % set(gcf, 'Name', ['cross-fly LR traces - ', regionName])
                    set(gcf, 'Name', ['cross-fly Left traces - ', regionName, ' block ', num2str(batBlock), ' Behav: ', char(stateNames(currentState))], 'NumberTitle', 'off', 'Renderer', 'painters')

                    yLims = get(gca, 'ylim');
                    fill([0 0 1.089 1.089], [yLims(1), yLims(2), yLims(2), yLims(1)], 'b', 'FaceAlpha', 0.3)
                    title(['cross-fly Left traces - ', regionName, ' block ', num2str(batBlock), ' Behav: ', char(stateNames(currentState))])
                    ...
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    % set(gcf, 'Name', ['cross-fly LR traces - ', regionName])
                    set(gcf, 'Name', ['cross-fly Right traces - ', regionName, ' block ', num2str(batBlock), ' Behav: ', char(stateNames(currentState))], 'NumberTitle', 'off', 'Renderer', 'painters')

                    yLims = get(gca, 'ylim');
                    fill([0 0 1.089 1.089], [yLims(1), yLims(2), yLims(2), yLims(1)], 'b', 'FaceAlpha', 0.3)
                    title(['cross-fly Right traces - ', regionName, ' block ', num2str(batBlock), ' Behav: ', char(stateNames(currentState))])
                    ...
                    %Report if X axis likely wonky
                    temp = allFlyLR(:,:,3, batBlock)';
                    temp2 = nansum( ~isnan(temp),1 );
                    temp3 = find( abs(temp2-nanmean(temp2)) > 1*nanstd(temp2) );
                    if ~isempty(temp3)
                        ['-# Caution: Dataset/s ',num2str(temp3),' (~#',num2str(chosenFlies(temp3)),') likely have different timescales #-']
                            %Manifests as funny looking averages, due to different timescales
                            %Note: This reporting might act funny if multiple blocks per fly present
                    end
            ...
            %amp calcs
            LRAmps = nan(size(allFlyLR, 1), size(allFlyLR, 4), 3); % [flyAmplitude, block, combined(1)/left(2)/right(3)]
            ...
            for batBlock = 1:size(allFlyLR, 4) %find(blockNums == blockType)
                for fly = 1:size(allFlyLR, 1)
                    for sideType = 1:3 %size(allFlyLR, 3) %maybe move to outer loop to save time if needed
        ...
        if conditionChoice(3)
            ...
            % lumTitles = {'Luminosity 1', 'Luminosity 2', 'Luminosity 3', 'Luminosity 4', 'Luminosity 5', 'Luminosity 6', 'Luminosity 7'};
            lumTitles = {'Lum 1', 'Lum 2', 'Lum 3', 'Lum 4', 'Lum 5', 'Lum 6', 'Lum 7'};
            %traces for each fly
            ...
            for block = 1:size(blockNums, 2) %find(blockNums == 1)
                ...
                % if allGraphs
                %     figure
                %     set(gcf, 'Name', ['Fly ', num2str(batStruct.blocks(block).fly), 'block ', num2str(blockNums(block)), ': Luminosities', ' - ', regionName], 'NumberTitle', 'off', 'Renderer', 'painters')
                % end
                for lum = 1:size(lumVols, 4)
                    ...
                    if allGraphs
                        ...
                        title(['Fly ', num2str(batStruct.blocks(block).fly), 'block ', num2str(blockNums(block)), ', Luminosity:', num2str(lum), ' - ', regionName, ' Behav: ', char(stateNames(currentState))])
            ...
            if allGraphs
                ...
                for f = 1:size(allFlyLum, 1)
                    for block = 1% 1:size(allFlyLum, 5)
                        ...
                        set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                        set(gcf, 'Name', ['fly ', num2str(chosenFlies(f)), ' block ', num2str(block), ' - all luminosity traces - ', regionName, ' Behav: ', char(stateNames(currentState))], 'Renderer', 'painters')
                        legend(lumTitles)
                        title(['fly ', num2str(chosenFlies(f)), ' block ', num2str(block), ' - all luminosity traces - ', regionName, ' Behav: ', char(stateNames(currentState))])
                    end
                end
            end
            ...
            if size(chosenFlies, 2) > 1
                for block = 1:size(allFlyLum, 5)
                    ...
                    hold on
                    for lum = 1:size(allFlyLum, 4)
                        plot(nanmean(allFlyLum(:,:,3,lum,block), 1), nanmean(allFlyLum(:,:,1,lum,block), 1), 'LineWidth', 3, 'Color', lumColours{lum})
                    end
                    ylabel('mean fluorescence (\DeltaF/F)')
                    xlabel('time (s)')
                    yLims = get(gca, 'ylim');
                    ...
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    set(gcf, 'Name', ['cross-fly luminosities, block ', num2str(block), ' - ', regionName, ' Behav: ', char(stateNames(currentState))], 'Renderer', 'painters')
                    title(['cross-fly luminosities, block ', num2str(block), ' - ', regionName, ' Behav: ', char(stateNames(currentState))])
                    legend(lumTitles)
                end
            end
        end
        ...
        if conditionChoice(4)
                ...
                for f = 1:size(allFlyFreq, 1) %:size(chosenFlies, 2)
                    for block = 1:size(allFlyFreq,5)
                        ...
                        for freq = 1:size(allFlyFreq, 4)
                            ...
                            xlabel('time (s)')
                            set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                            set(gcf, 'Name', ['fly ', num2str(chosenFlies(f)), ' block ', num2str(block), ' - all frequencies (', regionName, ')', ' Behav: ', char(stateNames(currentState))], 'Renderer', 'painters')
                            title(['fly ', num2str(chosenFlies(f)), ' block ', num2str(block), ' - all frequencies (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                ...
                for fly = 1:size(allFlyFreq, 1)
                    for block = 1:size(allFlyFreq, 5)
                        for freq = 1:size(allFlyFreq, 4)
                            ...
                            xlabel('time (s)')
                            set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                            set(gcf, 'Name', ['fly ', num2str(chosenFlies(fly)), ' block ', num2str(block), ' - ', num2str(matSave.batFrequencies(freq)), ' Hz (', regionName, ')', ' Behav: ', char(stateNames(currentState))], 'Renderer', 'painters')
                            title( ['fly ', num2str(chosenFlies(fly)), ' block ', num2str(block), ' - ', num2str(matSave.batFrequencies(freq)), ' Hz (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                        end
                    end
                end
            end
            ...
            for block = 1:size(allFlyFreq, 5)
                for freq = 1:size(allFlyFreq, 4)
                    if allGraphs && size(chosenFlies, 2) > 1
                        figure
                        hold on
                        ...
                        xlabel('time (s)')
                        set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                        set(gcf, 'Name', ['cross-fly block ', num2str(block), ' - ', num2str(matSave.batFrequencies(freq)), ' Hz (', regionName, ')', ' Behav: ', char(stateNames(currentState))], 'Renderer', 'painters')
                        title( ['cross-fly ', ' block ', num2str(block), ' - ', num2str(matSave.batFrequencies(freq)), ' Hz (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                    end
            ...
            if size(chosenFlies, 2) > 1
                for block = 1:size(allFlyFreq, 5)
                    ...
                    ylabel('mean fluorescence')
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    set(gcf, 'Name', ['cross-fly block ', num2str(block), ' - ', ' - all frequencies (', regionName, ')', ' Behav: ', char(stateNames(currentState))], 'Renderer', 'painters')
                    title(['Cross-fly frequencies - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                end
            end
        end
        ...
        for batBlock = 1:size(allFlyBlanks, 4) %find(blockNums == blockType)
            if size(chosenFlies, 2) > 1
                ...
                set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                % set(gcf, 'Name', ['cross-fly LR traces - ', regionName])
                set(gcf, 'Name', ['cross-fly blank traces - ', regionName, 'block ', num2str(batBlock), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))], 'NumberTitle', 'off', 'Renderer', 'painters')
                title(['cross-fly blank traces - ', regionName, 'block ', num2str(batBlock), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
            end
        ...
        if allGraphs
            for block = 1:size(LRAmps, 2)
                ...
                title(['Cross-Fly LR amplitudes - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                set(gcf, 'Renderer', 'painters', 'Name', ['Cross-Fly LR amplitudes - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
            end
        end
        ...
        if allGraphs
            for block = 1:size(lumAmps, 3)
                ...
                title(['Cross-Fly lum amplitudes - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                set(gcf, 'Renderer', 'painters', 'Name', ['Cross-Fly lum amplitudes - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
            end
        end
        ...
        if allGraphs
            for block = 1:size(freqAmps, 3)
                ...
                title(['Cross-Fly freq amplitudes - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
                set(gcf, 'Renderer', 'painters', 'Name', ['Cross-Fly freq amplitudes - block ', num2str(block), ', (', regionName, ')', ' Behav: ', char(stateNames(currentState))])
            end
        end
        ...
        %Debug saving of overStruct
        if debugOverSave
            savName = [savSeed,'_overStruct_state_',num2str(currentState),'_region_',regionName];
            save(fullfile(overSaveFolder,savName),'overStruct','chosenFlies','chosenBlocks','currentState','regionName')
            disp(['-- Debug overStruct saved as ',savName,' to ',overSaveFolder,' --'])
        end
    ...
    %% Cross-ROI analyses
    if size(roiList, 2) == 1
        disp('analyses complete! for more graphs add more ROIs or background')
    else
        ...
        for block = 1:size(regionisedAmps, 3)
            ...
            set(gcf, 'Renderer', 'painters', 'Name', ['Cross Fly LR amps - block ' num2str(block)])
        ...
        for block = 1:size(regionisedAmps, 3)
            ...
            for stimType = [2,3]
                ...
                if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4 %size(LRData, 2) < 4
                        %Note: Second boolean here based on guesstimate of system for identifying non-existing blocks in fly data
                    % disp('not enough flies for appropiate stats')
                    LRDescription = 'stats not appropriate for <4 flies';
                else
                    %stats
                    LRData = squeeze(regionisedAmps(:, stimType, block, 1, :)); %Stage 1
                    %---
                    LRData = mattNanRemover( LRData, 'checkSize', size(chosenFlies, 2) );
                    %Old, non-function form of NaN removal
                    %{
                    %Matt data trimming to reduce issues with norm tests
                    LRData = LRData( [ ~isnan(LRData(:,1)) & ~isnan(LRData(:,2)) ] , : ); %Matt addition, seems necessary if dissimilar block-ness
                        %Note: Relies on hardcoded 2-column nature
                        %Secondary note: If any processes attempt to relate row # -> fly identity this will cause issues
                    %Report
                    if size(LRData,1) ~= size(regionisedAmps,1)
                        disp(['-# Caution: Cross-fly data for LR contained NaN rows, which were removed (N=',num2str(size(regionisedAmps,1)),'->',num2str(size(LRData,1)),') #-']) %e.g. One or more flies doesn't have block 1 etc
                    end
                    if size(LRData,1) < 4
                        LRDescription = 'stats not appropriate for <4 flies';
                        disp(['-# Caution: Post-NaN removal size <4; Cannot stats #-'])
                        return
                    end
                    if isempty(LRData)
                        ['-# Alert: No apparent cross-fly LR data due to NaN removal #-']
                        crash = yes %Might be common?
                    end
                    %}
                    %---
                    nonNormCheck = 0;
                ...
                if stimType == 2
                    title(['left Stimulus, block ', num2str(block)])
                    set(gcf, 'Name', ['left Stimulus, block ', num2str(block)])
                elseif stimType == 3
                    title(['right stimulus, block ', num2str(block)])
                    set(gcf, 'Name', ['right stimulus, block ', num2str(block)])
                end
            end
            % xticks([1,2])
        ...
        if size(roiList, 2) >= 2 && ( any(ismember(roiList, 'background')) || any(ismember(roiList, 'bg')) )
            ...
            for block = 1:size(regionisedAmps, 3)
                ...
                for reg = 1:size(regionisedAmps, 5) - 1
                    ...
                    if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4
                        LRDescription = 'stats not appropriate for <4 flies';
                    else
                        %stats
                ...
                ylabel('Amplitude (peak\DeltaF/F)')
                title(['left/right vs bg(combined) per ROI, block', num2str(block)])
                set(gcf, 'Name', ['left/right vs bg(combined) per ROI, block', num2str(block)])
                set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
            end
        end

        %comparing ROI to background - only works for single non-background ROI with background.
        if size(roiList, 2) == 2 && ( any(ismember(roiList, 'background')) || any(ismember(roiList, 'bg')) )
            bgIndex = ismember(roiList, 'background');
            if ~any(bgIndex) % if 'background' wasn't the name used (currently only other option is 'bg')
                bgIndex = ismember(roiList, 'bg');
            end
            ...
            for block = 1:size(regionisedAmps, 3)
                ...
                title([roiList{~bgIndex}, ' vs background'])
                set(gcf, 'Name', [roiList{~bgIndex}, ' vs background'])
                set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
            end
        end
        ...
        for block = 1:size(regionisedAmps, 3)
            ...
            for stimType = 1:7
                if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4
                    lumDescription = 'stats not appropriate for <4 flies';
                else
                    lumData = squeeze(regionisedAmps(:, stimType, block, 2, :));
                    %---
                    lumData = mattNanRemover( lumData, 'checkSize', size(chosenFlies, 2) );
                    %---
            ...
            ylabel('Amplitude (peak\DeltaF/F))')
            title(['crossFly luminosities ROI-based comps, block ', num2str(block)])
            set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
        end
        ...
        if size(roiList, 2) >= 2 && ( any(ismember(roiList, 'background')) || any(ismember(roiList, 'bg')) )
            for block = 1:size(regionisedAmps, 3)
                ...
                for reg = 1:size(regionisedAmps, 5) - 1 %-1 because don't want to plot bg as its own roi
                    if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4 
                        lumDescription = 'stats not appropiate for <4 flies';
                    else
                        %stats
                        ...
                        %---
                        lumData = mattNanRemover( lumData, 'checkSize', size(chosenFlies, 2), 'suppressReport', 1 );
                        %---
                ...
                ylabel('Amplitude (peak\DeltaF/F))')
                set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                title(['crossFly luminosities, within ROI comps, block ', num2str(block)])
            end
        end
        ...
        for block = 1:size(regionisedAmps, 3)
            ...
            xticklabels(roiList)
            %spiffy sets
            title(['Cross Fly Freq amps - block ' num2str(block)])
        end
        ...
        for block = 1:size(regionisedAmps, 3)
            ...
            for stimType = 2:4
                if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4
                    freqDescription = 'stats not appropiate for <4 flies';
                else
                    %stats
                    freqData = squeeze(regionisedAmps(:,stimType, block, 3, :));
                    %---
                    freqData = mattNanRemover( freqData, 'checkSize', size(chosenFlies, 2) );
                    %---
            ...
            ylabel('Amplitude (peak\DeltaF/F))')
            set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
            title(['crossFly block ', num2str(block), ' - frequencies'])
        end
        ...
        if size(roiList, 2) >= 2 && ( any(ismember(roiList, 'background')) || any(ismember(roiList, 'bg')) )
            for block = 1:size(regionisedAmps, 3)
                ...
                ylabel('Amplitude (peak\DeltaF/F)')
                set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                title(['crossFly block ', num2str(block), ', frequencies vs bg'])
            end
        end

        %% traces
        %LR
        regionisedLR = nan(3, size(allFlyLR, 2), size(allFlyLR, 3), size(allFlyLR, 4), size(overStruct, 2)); % [ combined(1)/left(2)/right(3), vol, allData(1)/photo(2)/time(3)/activeOnly(4)/inactive(5), block, ROI ]
    ...
    elseif size(regionisedAmps, 3) == 1
        ...
        for reg = 1:size(regionisedAmps, 5)
            for condition = 1:size(regionisedAmps, 4)
                ...
                for feature = featSize %1:size(regionisedAmps, 3)
                    % featCount = 1; %used later for saving to blockStat variables
                    if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4
                        currentDescription = 'stats not appropiate for <4 flies';
                    else
                        currentData = [];
                        for block = 1:size(regionisedAmps, 3)
                        ...
                        %---
                        currentData = mattNanRemover( currentData, 'checkSize', size(chosenFlies, 2) );
                        %---
        ...
        for reg = 1:size(regionisedAmps, 5) - 1
            ...
            for block = 1:size(regionisedAmps, 3)
                if size(chosenFlies, 2) < 4 || sum( ~isnan( squeeze( regionisedAmps(:,1,block,1,1) ) ) ) < 4
                    LRDescription = 'stats not appropiate for <4 flies';
                else
                    %stats
                    LRData = [squeeze(regionisedAmps(:,[2,3],block,1,reg)), squeeze(regionisedAmps(:,1,block,1,end))];
                    %---
        ...
        for reg = 1:size(regionisedAmps, 5) -1
            ...
            for block = 1:size(regionisedAmps, 3)
                ...
                %barX = lumOffset + block; %Doesn't account for 8-wide nature of below errorbar
                %barX = [lumOffset,nanmean(diff(lumOffset))] + block; %Bootleg add another element at same spacing; Doesn't actually work properly
                barX = linspace(0.65,1.35,8) + (block-1); %Empirical, based on native errorbar placement with 8 elements
                errorbar(barX, [squeeze(nanmean(regionisedAmps(:,[1:7], block, 2 , reg), 1)) , squeeze(nanmean(regionisedAmps(:,7, block, 2, end)))],  [regionisedAmpSEM(:,[1:7], block, 2, reg), regionisedAmpSEM(:,7, block, 2, end)], 'LineStyle', 'none', 'Color', 'k')
                if doDots
                    scatter(barX, [squeeze(regionisedAmps(:,[1:7], block, 2 , reg)) , squeeze(regionisedAmps(:,7, block, 2, end))], 10, 'k', 'filled')
                end
...
end %behavioural state loop end
...
disp(['Total elapsed time: ',num2str(durationTime/60),'m'])

%finis

if automatedSaveFigures
    automatedPlotSaver( autoFigPath , additionalFigParams, 'clearOldFigures', 1, 'suppressRenamingMessages', 1)
end


%%

function [thisData] = mattNanRemover(thisData,options)

arguments
    thisData double
    options.checkSize double = [];
    options.suppressReport double = 0
end

    checkSize = options.checkSize; %Size to check against if requested
    suppressReport = options.suppressReport;

    %Matt data trimming to reduce issues with norm tests
    keepInds = [];
    for n = 1:size(thisData,1)
        if ~any( isnan(thisData(n,:)) ) %Check all columns
            keepInds = [keepInds;n]; %Ugly but works
        end
    end
    %thisData = thisData( [ ~isnan(thisData(:,1)) & ~isnan(thisData(:,2)) ] , : ); %Matt addition, seems necessary if dissimilar block-ness
        %Note: Relies on hardcoded 2-column nature
    thisData = thisData( keepInds , : );   
        %Secondary note: If any processes attempt to relate row # -> fly identity this will cause issues
    %Report
    if ~isempty(checkSize) && size(thisData,1) ~= checkSize && ~suppressReport
        disp(['-# Caution: Cross-fly data for LR contained NaN rows, which were removed (N=',num2str(checkSize),'->',num2str(size(thisData,1)),') #-']) %e.g. One or more flies doesn't have block 1 etc
    end
    if size(thisData,1) < 4
        disp(['-# Caution: Post-NaN removal size <4 #-'])
    end
    if isempty(thisData)
        ['-# Alert: No apparent cross-fly LR data post NaN removal #-']
        crash = yes %Might be common?
    end
end