...
% V1...Mk1 - (10/10/25), many additions made leading up to thesis submission (this version made lots of unnecessary graphs), sequence orders are now based on what side last stimulus was presented on rather than seq_eff_order (for side-sepped profiles - averaging pending) 
% V1...Mk2 - (29/10/25), history rank option added, more plot controls added
% V1...Mk3 - (), background subtraction implemented (using average trace response of each fly), multiState functionality added, multiROI option added (+all ROIs generated outside loop first).
...
sideTraces = 1; % option to plot average traces to all histories ending on left vs ending on right for every block
loopProfiles = 1; % option to generate profiles for every block (within the main collection loop)
fullProfiles = 0; %option to plot 16-history based profiles
limitProfiles = 1; %option to plot 4-hitsory based profiles and associated transients (RRRR, AAAR, RRRA, AAAA)
useMaxOnly = 1; %amp specification to use the max value

doRankMethod = 1; % option to rank the histories based on amplitudes

%bg options
bgSubtract = 1; %choice to subtract background trace from structural traces for each flies average trace

%behaviour
behavSep = 0; % choice to split analysis by behavioural state
useBT = 1; % option to use behaviour generated from BTData, if off will use pixelSubtraction (may be changed eventually if choice made in an upstream script)
behavStates = -1; %[0,1]; %list of states to probe for, -1 indicates no sep (might become dynamic eventually, or even string-based, but just # that relates to behavSequence for now)
...
ROI = 1;
% regionName = 'bg'; %leftMB, background, 
roiList = {'leftMB'};
...
roiStruct = struct;
if ROI
    for reg = 1:size(roiList, 2)
        roiStruct(reg).regionName = roiList{reg};
        roiStruct(reg).roiFlies = struct;
        
        regionName = roiList{reg};

        % roiFlyCount = 1;
        for roiFly = 1:size(chosenFlies, 2)
            roiStruct(reg).roiFlies(roiFly).roiBlocks = struct;
            ...
            for roiBlock = 1:size(FLIES(roiFly).BLOCKS, 2)
                ...
                roiStruct(reg).roiFlies(roiFly).roiBlocks(roiBlock).roiMap = roiMap;
                % roiFlyCount = roiFlyCount + 1;
            end
        end
        % roiFlyCount = 1; %resetting roiCount for use in main loop
    end
end
...
%using seq_eff_order to generate side-separated dependency orders
...
sideNames = {'side average', 'left', 'right'};
SEDes = SETitles;
SEDes{17} = 'Blanks';

saveVars = who;
saveVars = [saveVars; 'saveVars'; 'currentState']; %adding other necessary variables
...
for currentState = behavStates
    ...
    for region = 1:size(roiList, 2)
        clearvars('-except', loopVars{:}); %clearing variables generated in loop
        ...
        for fly = 1:size(FLIES, 2) %[1,2,5,7] - 'good' ATR flies?
            ...
            for b = 1:size(currentFly, 2)
                ...
                flyFolder = ['fly', num2str(blockRecords.FlyOnDay), '_exp', num2str(blockRecords.Block), '_', currentDate];
                ...
                %applying ROI
                if ROI
                    roiMap = roiStruct(region).roiFlies(fly).roiBlocks(b).roiMap;
                ...
                % turning splicedRandomSequence into numbered code
                ...
                eventTraces = nan(4, 40, size(splicedRandomSequence, 3)); % [calcium(1)/photo(2)/time(3)/behav(4), volume, eventNum] 
                ...
                for eventNum = 1:size(splicedRandomSequence,3)
                    if find(isnan(splicedRandomSequence(:,:,eventNum)), 1, 'first')
                        ...
                        for i = splicedRandomSequence(2,:,eventNum)
                            ...
                            eventTraces(2,lastVol:currentSize + lastVol - 1, eventNum) = allIExtras( 2, 1:currentSize, i); %photo
                            eventTraces(3,lastVol:currentSize + lastVol - 1, eventNum) = allIExtras( 1, 1:currentSize, i); %time
                            eventTraces(4,lastVol:currentSize + lastVol - 1, eventNum) = repmat(behavData(i), 1, currentSize); %behav
                ...
                %applying savitsky-golay filter... and maybe others soon
                ...
                % splicing by state
                statedEventTraces = nanmean(eventTraces(4,:,:), 2);
                eventTraces(:, :, squeeze(statedEventTraces) ~= currentState) = nan;
                eventDurationTimes(squeeze(statedEventTraces) ~= currentState, :) = nan;
                ...
                historyTraces = nan(4, size(eventTraces, 2), ceil(size(seqOrder,2)/7), max(seqOrder)+2); % [datatype spec, vols, history repition, history type] datatypes = [calciumData(1), photodiode(2), rawTimes(3), durationTimes(4)] +2 because 0 can't be an indice and blanks
                traceNums = ones(1, size(historyTraces, 4));
                for eventNum = 1:size(seqOrder, 2)
                    currentHistory = seqOrder(eventNum);
                    if currentHistory == -2
                        % historyTraces(1:size(eventTraces, 1),:, traceNums(end),end) = eventTraces(1:3,:,eventNum);
                        historyTraces(1:3,:, traceNums(end),end) = eventTraces(1:3,:,eventNum);
                        historyTraces(4, :, traceNums(end), end) = eventDurationTimes(eventNum, :);
                        traceNums(end) = traceNums(end) + 1;
                    else
                        historyTraces(1:3,:, traceNums(currentHistory + 1), currentHistory + 1) = eventTraces(1:3,:,eventNum);
        ...
        if limitProfiles
            ...
            %plotting side-averaged RRRR, RRRA, AAAR, AAAA - for thesis(for real this time)
        ...
        for block = 2:size(allFlyAvgFeats, 4)
            if size(chosenFlies, 2) < 4 
                statDescription = 'stats not appropiate for <4 flies';
            else
                %stats
        ...
        if limitProfiles
            ...
            for block = 2%2:size(allFlyAvgFeats, 4)
                if size(chosenFlies, 2) < 4 
                    ...
                    currentData = squeeze(allFlyAvgFeats(:,:, 1,block));
                    ...
                    for group = 1:size(SESig, 1)
                        statDescription = [statDescription, [num2str(SESig(group, 1)), 'v', num2str(SESig(group, 2)), ': ', num2str(SESig(group, 6)), newline]];
                        % statDescription = [statDescription, [num2str(SEDes{SESig(group, 1)}), 'v', num2str(SEDes{SESig(group, 2)}), ': ', num2str(SESig(group, 6)), newline]];
                        % tf is SEDes??
                    end
                end
        ...
        if doRankMethod
            for block = 2:size(allFlyAvgFeats, 4)
                ...
                for side = [1,2]
                    ...
                    %cross fly
                    avgResponse = [squeeze(nanmean(allFlyFeatures(:,index,1,block), 1)),  squeeze(nanmean(allFlyFeatures(:,end, 1,block),1))]; % [amps (listed in order of dec2bin), + blankAmp]
                    %squeeze(nanmean(allFlyAvgFeats(:,index, 1, block), 1));

                    [~, rank] = sortrows(avgResponse', -1);

                    sidedRank = [sidedRank, rank];

                    % rankDescription = [];
                    % for hist = 1:size(rank, 1)
                    %     if rank(hist) == 17 % || rank(hist) > size(SETitles, 1)
                    %         rankDescription = [rankDescription, 'blank', newline];
                    %         why
                    %     else
                    %         rankDescription = [rankDescription, SETitles{rank(hist)}, newline];
                    %         % rankDescription = [rankDescription, dec2bin(rank(hist) - 1, 5), newline];
                    %     end
                    % end
                ...
                %side-averaged
                avgResponse = squeeze(nanmean(allFlyAvgFeats(:,:,1,block), 1));
                [~, avgFeatRank] = sortrows(avgResponse', -1);

                % rankDescription = [];
                % for hist = 1:size(avgFeatRank, 1)
                %     if avgFeatRank(hist) == 17 % || avgFeatRank(hist) > size(SETitles, 1)
                %         rankDescription = [rankDescription, 'blank', newline];
                %         % why
                %     else
                %         rankDescription = [rankDescription, SETitles{rank(hist)}, newline];
                ...
                allFlyFeatRank = nan(size(allFlyAvgFeats, 1), 17);
                %Per fly
                for fly = 1:size(allFlyAvgFeats, 1)
                    avgResponse = squeeze(allFlyAvgFeats(fly,:,1,block));
                    [blarg, rank] = sortrows(avgResponse', -1);

                    for history = 1:size(rank, 1)
                        allFlyFeatRank(fly, history) = find(rank==history, 1, 'first');
                    end
                end
            end
        end
        ...
        %saving for crossROI comparisons
        overStruct(region).allFlyTraces = allFlyTraces;
        overStruct(region).allFlyAvgFeats = allFlyAvgFeats;

    end %roi loop end
    %crossROI analyses here

    %% region-based Traces
    regionisedTraces = nan([size(allFlyTraces, 1:5), size(roiList, 2)] ); % 6-dimensions - [fly(1), volume(2), dataType(3), history(4), block(5), region(6)]
    for roi = 1:size(regionisedTraces, 6)
        % for block = 1:size(regionisedTraces, 5)
        %     regionisedTraces(:,:,:,:,block,roi) = overStruct(roi).allFlyTraces;
        % end
        regionisedTraces(:,:,:,:,:,roi) = overStruct(roi).allFlyTraces;
    end

    %background subtraction
    if size(roiList, 2) < 2 || ~( roiList{end} == "background" || roiList{end} == "bg" )
        disp('ROI based comparisons not conducted: To access this part of the code, list at list at least one structure with background last')
    else
        %background subtraction
        for roi = 1:size(roiList, 2) - 1
            for block = 1:size(regionisedTraces, 5)
                regionisedTraces(:, :, 1, :, block, roi) = regionisedTraces(:, :, 1, :, block, roi) - regionisedTraces(:, :, 1, :, block, end);
            end
        end
        % ^ insert trace comparison plots - initial trace + bgSubTrace + bg trace

        % generating profile
        %amp calc (from each flies average trace)
        regionisedAmps = nan([size(regionisedTraces, 1), 3, 17, size(regionisedTraces, [5,6])]); % [fly(1), dataType-side(2), history(3), block(4), region(5)], datatype = [averaged(1)/left ending(2)/right ending (3)]
        for roi = 1:size(regionisedTraces, 6)
            for fly = 1:size(regionisedTraces, 1)
                for his = 1:17 %size(regionisedTraces, 4)
                    for block = 1:size(regionisedTraces, 5)
                        for side = 1:3
                            if his == 17 %blank same regardless of side
                                currentTrace = regionisedTraces(fly, :, 1, 33, block, roi);
                            elseif side == 1 %averaged
                                leftTrace = regionisedTraces(fly, :, 1, leftIndex(his), block, roi);
                                rightTrace = regionisedTraces(fly, :, 1, rightIndex(his), block, roi);
                                
                                currentTrace = nanmean([leftTrace; rightTrace], 1);
                            elseif side == 2 %left
                                currentTrace = regionisedTraces(fly, :, 1, leftIndex(his), block, roi);
                            elseif side == 3 %right
                                currentTrace = regionisedTraces(fly, :, 1, rightIndex(his), block, roi);
                            end
                            ...
                            % [fly(1), dataType(2), history(3), block(4), region(5)]
                            %storing amps
                            if his == 17
                                regionisedAmps(fly, :, end, block, roi) = amp;
                            else
                                regionisedAmps(fly, side, his, block, roi) = amp;
                            end
        ...
        regionisedSEM = nanstd(regionisedAmps, [], 1)/sqrt(size(regionisedAmps, 1));

        %plotting bg subtracted profile
        for roi = 1:size(roiList, 2) - 1
            for block = 2:size(regionisedAmps, 4) %consider option to remove first block if all elements filled with nans maybe, or just something to skip it
                for side = 1:3
                    if size(chosenFlies, 2) < 4
                        statDescription = 'stats not appropiate for <4 flies';
                    else
                        %stats
                        % currentData = squeeze(allFlyAvgFeats(:,seq_eff_order(5), 1,block));
                        currentData = squeeze(regionisedAmps(:, side, :, block, roi));
                    ...
                    figure
                    hold on
                    bar(1:16, squeeze(nanmean(regionisedAmps(:, side, 1:16, block, roi), 1)))
                    plot(1:16, squeeze(regionisedAmps(:, side, 1:16, block, roi)))
                    % scatter(1:4, squeeze(allFlyAvgFeats(:, [1,8,9,16], 1, block)), 'filled')
                    errorbar(1:16, squeeze(nanmean(regionisedAmps(:, side, 1:16, block, roi), 1)), squeeze(regionisedSEM(1, side, 1:16, block, roi)), 'LineStyle', 'none', 'Color', 'k')
                    line([1,16], repmat(nanmean(regionisedAmps(:, side, end, block, roi), 1), 1, 2), 'LineWidth', 2, 'Color', 'r')
                    set(gca, 'LineWidth', 2, 'TickDir', 'out', 'FontWeight', 'bold', 'FontSize', 12)
                    % figMax = get(gca, 'ylim');
                    % figMax(1) = [];
                    % text(barX(end), figMax/2, LRDescription)
                    ...
                    title(['crossFly, background substracted ', sideNames{side}, ' block ', num2str(block), ' (', roiList{roi}, ') ', char(stateNames(currentState))])
                    set(gcf, 'Renderer', 'painters')
